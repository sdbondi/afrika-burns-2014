...
class QProjectMWidget : public QGLWidget
{
    ...
    // Timeout before mouse cursor disappears (ms)
		...
		QProjectMWidget ( const std::string & config_file, QWidget * parent, QMutex * audioMutex = 0)
				: QGLWidget ( parent ), m_config_file ( config_file ), m_projectM ( 0 ), m_audioMutex ( audioMutex ), m_mouseTimer ( 0 ), m_prismatic_input(0)
		{
	...
	protected slots:
      ...
      if (PRISMATIC_INPUT_TYPE == PRISMATIC_MOUSE_INPUT && m_prismatic_input != 0)
      {
        ...
        static_cast<PrismaticMouseInputAdapter*>(m_prismatic_input)->UpdateCoords(new_x, new_y);
      }
		}
    ...
    inline void handleMouseButtons(QMouseEvent* event)
    {
